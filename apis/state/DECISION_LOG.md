...
- **Decision:** Add `seed_from_db_positions(positions, last_known_equity)` to `PaperBrokerAdapter` and call it from the lazy-init branch in `run_paper_trading_cycle` (right after `app_state.broker_adapter = PaperBrokerAdapter(...)`). The helper rebuilds in-memory `_positions` from a list of `(ticker, quantity, avg_entry_price)` tuples and pins `_cash = last_known_equity - cost_basis` so the next `sod_equity_captured` reads the operator's real prior-close equity rather than the cold-start $100k. Module-level helper `_seed_paper_broker_from_db(broker, cycle_id)` resolves the inputs by querying `positions WHERE status='open'` and the most recent `portfolio_snapshots.equity_value`. Both layers are idempotent (skip already-seeded tickers) and fire-and-forget (any DB error → WARN, cycle proceeds). Backwards-compat env knob `phase81_broker_sod_reseed_enabled` (default True).
- **Alternatives considered:**
  - **(a) Unwind the 5 c7 BUYs that fired against the phantom $100k baseline.** Rejected. The 5 BUYs ($77k notional) executed at real Wed prices; unwinding them locks in slippage + transaction-cost noise and produces 5 extra closed rows with intra-day round-trip P&L. The operator's "Recommend (b)" guidance from the 2026-05-13 morning report names re-seed as the preferred remediation; (a) gets retired with this DEC.
  - **(b) Reseed at API startup only (`apps/api/main.py::_load_persisted_state`).** Rejected as INSUFFICIENT. The lifespan startup only fires when the API container boots; the PaperBrokerAdapter is lazy-initialized at the FIRST paper cycle each container lifecycle. On a Docker engine recovery the API boots without a broker, then the next cycle's lazy-init creates a fresh `PaperBrokerAdapter` that misses the API-side seed. Reseed at the lazy-init site is the only place that catches every restart path.
  - **(c) Mutate `portfolio_state.cash` to `equity_value` in `_load_persisted_state` and bypass the broker entirely.** Rejected. Cash is the broker's primary state; making the API-side restore source-of-truth re-creates the historical "phantom broker positions" bug class that Phase 64 + Phase 75 + Phase 77 specifically closed.
- **Rationale:** Tue close equity $117,432 / DB cost basis $75,193 / cash $42,239. Wed 19:25Z Docker recovery rebuilt the worker container; the lazy-init at the first cycle handed back a PaperBrokerAdapter at the cold-start `$100k cash + 0 positions`. The 19:30Z c7 SOD capture then read `equity = $100k` (not the real $117k) and fired 5 BUYs sized off that baseline. Reseeding at lazy-init means the SOD capture sees a broker whose `cash + positions == last_snapshot.equity_value`, which is byte-for-byte the pre-restart state.
- **Consequence:**
  - On a fresh worker start with N open DB positions and a recent snapshot, the broker `get_account_state()` now reports `cash_balance = snapshot.equity - sum(entry * qty)` and `equity = snapshot.equity`. Next cycle's SOD capture writes the correct value.
  - New `phase81_broker_sod_reseeded_from_db` INFO log line carries `prior_cash`, `new_cash`, `cost_basis`, `seeded` for daily verification.
  - 6 unit tests in `TestPhase81ABrokerReseed` lock the helper's contract; 1 round-trip test verifies SOD-equity matches the simulated Tue close within $1 rounding tolerance.
  - The c7 BUYs from the 2026-05-13 incident are NOT unwound (per operator's recommendation b) — they stand as legitimate Wed-priced opens against what is now a correctly reconciled baseline.

### DEC-082: Phase 81-B — Universal OPEN stacking guard at paper_trading.py
- **Decision:** Extend the Phase 79 rebalance-only OPEN filter to ALL OPEN actions (regardless of which strategy emitted them). Insert a new gate AFTER the Phase 65b intra-cycle churn guard and BEFORE the per-action risk validation loop. Same two-axis check as Phase 79: drop any OPEN whose ticker is in `portfolio_state.positions` with qty > 0 OR in `_broker.list_positions()`. Backwards-compat env knob `phase81b_open_stacking_guard_enabled` (default True).
- **Alternatives considered:**
  - **(a) Lift the rebalance-only check into a strategy-agnostic helper inside `services/portfolio_engine/service.py`.** Rejected: the broker snapshot is only available inside `run_paper_trading_cycle`. Lifting it would require passing the broker handle into the portfolio engine, tightening coupling for a defence-in-depth check.
  - **(b) Extend the per-action risk validation `validate_action` to consult positions.** Rejected: `validate_action` is the gate-keeper for risk *rules* (limits, kill switch, inactive ticker). Stacking is a portfolio invariant, not a risk rule; mixing concerns muddies the rule-engine taxonomy.
  - **(c) Skip Phase 81-B and rely on Phase 79 + Phase 65b alone.** Rejected: Phase 79 only covers `rebalance_open`; `ranked_buy_signal` / `momentum_v1` / `theme_alignment_v1` would still stack if `portfolio_state.positions` were stale relative to broker (the same staleness window Phase 79 was built for).
- **Rationale:** The 2026-05-13 c7 incident shows that even non-rebalance OPENs (the 5 BUYs include INTC, AMZN, AMD, MU, UNH — all listed as rebalance candidates on Tue) can stack against the broker's reseed-recovered positions if the strategy proposes them on the recovery cycle. Phase 81-A reseeds the broker; Phase 81-B ensures the strategy can't immediately re-stack on top of that reseed.
- **Consequence:** Any OPEN — from any strategy — whose ticker already shows in state OR broker is dropped at the call site with a `phase81b_open_stacking_skipped` INFO log line. CLOSE / TRIM unaffected. 7 unit tests in `TestPhase81BOpenStackingGuard` cover ranked_buy_signal, momentum_v1, theme_alignment_v1, mixed batches, and the disable flag.

### DEC-083: Phase 81-C — `realized_pnl` fallback in `_persist_positions` close-loop
- **Decision:** In `_persist_positions` close-loop, after the existing `ct_by_ticker` lookup, when `row.realized_pnl is None` AND `row.quantity > 0` AND `row.entry_price` exists, compute `realized_pnl = (exit_d - entry_d) * quantity`, where `exit_d` prefers `row.exit_price` (if already set), then falls back to `row.market_value / row.quantity`. Emit `phase81c_realized_pnl_fallback_applied` INFO line so downstream attribution can flag proxy vs exact P&L. Flag-gated via `phase81c_realized_pnl_fallback_enabled` (default True).
- **Alternatives considered:**
  - **(a) Force every close to go through the ClosedTrade ledger.** Rejected: broker-sync close (when broker no longer reports a ticker but no CLOSE action fired) is a legitimate code path triggered by external broker actions (e.g. manual operator close on the broker side, paper-broker `_update_position` clamping). Forcing all closes through the ledger requires intercepting the broker-sync removal logic and synthesizing a ClosedTrade — significantly more invasive and risks double-counting.
  - **(b) Backfill the 169 historical NULL rows.** Filed as an optional follow-up. Not in this DEC because the fallback uses the row's last `market_value` as an exit proxy, and historical `market_value` is the stale snapshot value (computed at close time), not the actual exit price — so the backfill would attach proxy values to old rows without an explicit "proxy" marker. Better to let the fix go forward-only and accept the historical gap as documented.
  - **(c) Compute `exit_price` from the latest `position_history` row instead of `market_value`.** Rejected: identical numerical source (both derive from `current_price * quantity`), but adds a JOIN to the writer's hot path.
- **Rationale:** 169 lifetime closed rows have `realized_pnl=NULL` because the writer only consults the in-cycle `ClosedTrade` list. When a position closes via broker-sync (no CLOSE action emitted), no ClosedTrade is generated, so the close-loop's `if ct is not None` branch is skipped and the row is written with NULL P&L. The fallback uses the row's own market_value as a best-effort exit price — correct when broker reconciliation removed the ticker because it was actually liquidated externally, approximately correct (within one snapshot cycle's drift) when the close was driven by a paper-broker internal action.
- **Consequence:** Going forward, broker-sync closes write `realized_pnl` and `exit_price` proxies instead of NULL. New log line `phase81c_realized_pnl_fallback_applied` lets attribution queries distinguish exact vs proxy P&L. 6 unit tests in `TestPhase81CRealizedPnlFallback`.

### DEC-084: Phase 81-D — Docker Desktop Autostart Blocker durable mitigation
- **Decision:** Land a Windows-side watchdog (`apis/scripts/windows_docker_watchdog.ps1`) that runs as a Scheduled Task and: (1) probes the Docker engine pipe via `docker version`, (2) starts Docker Desktop if the pipe is unresponsive, (3) runs `docker compose up -d` from the project's compose dir if core containers are missing, (4) restarts the worker container if the `scheduler_heartbeat` is stale. Companion `register_watchdog_task.bat` registers two triggers (at-logon + every 5 min). Container-side healthcheck also tightened to require BOTH the scheduler heartbeat AND main-thread heartbeat (Phase 71 was scheduler-only).
- **Alternatives considered:**
  - **(a) Move the entire stack to Linux/WSL2 + systemd.** Rejected: too large a migration for one incident class; out-of-scope for Phase 81.
  - **(b) Document accepted risk and rely on the operator's morning check.** Rejected: explicitly listed in the 2026-05-13 HEALTH_LOG as a HIGH-priority action because the same incident has recurred multiple times since 2026-04-15. Manual recovery loses a full trading day each time.
  - **(c) Add a Docker Compose-level autoheal sidecar that restarts containers.** Rejected: cannot recover from engine-down (the sidecar IS a container).
- **Rationale:** The Phase 71 in-container healthcheck is structurally unable to recover from the "engine pipe never exposed" scenario because the container itself isn't running. The Tue 19:30Z → Wed 19:25Z (~24h) outage matches that pattern. Only a host-level watchdog outside the engine's failure domain can detect + remediate.
- **Consequence:** Two new files (`apis/scripts/windows_docker_watchdog.ps1` + `apis/scripts/register_watchdog_task.bat`). One-shot mode default for the 5-min trigger; loop mode available for an at-logon perpetual run. Watchdog log at `C:\ProgramData\APIS\watchdog.log` with one JSON event per action. Operator must run `register_watchdog_task.bat` ONCE as Administrator. Container-side `docker-compose.yml` healthcheck tightened (Phase 71 + main heartbeat); `start_period` raised 120s → 180s to absorb the Alembic-migration + state-restore startup window.

### DEC-085: Phase 81-E — Refute the dual-invocation phase-split hypothesis
- **Decision:** Document in `state/HEALTH_LOG.md` that the 2026-05-13 phase-split hypothesis — "paper_trading.py runs TWO write phases per cycle each generating its own cycle_id" — is REFUTED by code inspection. `run_paper_trading_cycle` generates exactly ONE `cycle_id = uuid.uuid4().hex` per invocation (paper_trading.py:737) and that same id is threaded through `_persist_portfolio_snapshot`, `_persist_position_history`, `_persist_positions`, AND `_persist_orders_and_fills`. The orders writer in particular accepts the FULL list of `approved_requests` + `execution_results` in one call — BUYs and SELLs cannot acquire different cycle_ids via this code path.
- **Alternatives considered:**
  - **(a) Add a diagnostic line that logs the cycle_id at every write site.** Filed as a follow-up if the dual cycle_id pattern recurs after this DEC; logging surface is already adequate to investigate (every persist helper carries `cycle_id=...`).
  - **(b) Continue the operator-Windows-host external-writer probe.** Closed without further investigation. The Phase 70 paper cycle lock + the in-cycle single-uuid invariant + the idempotency key shape `{cycle_id}:{ticker}:{side}` together mean any second-cycle_id source must necessarily be a separate process writing to the same `orders` table — which would have to be intentional (no production cron path does this).
- **Rationale:** The action item asked the worker to *confirm* the phase-split hypothesis. Code inspection refutes it. The most likely remaining explanation for two cycle_ids on the same trading slot is that the BUYs and SELLs were emitted in two ADJACENT cycle slots (e.g. cycle 7 vs cycle 7-retry-on-misfire-grace) and the HEALTH_LOG's `gid` columns conflated them. The 13 deep-dives of operator-Windows-host probing can be retired — but the actual root cause analysis should be re-opened next time the pattern is observed, with a per-cycle log of `cycle_id` + `wall_clock` to disambiguate adjacent-cycle attribution.
- **Consequence:** Phase 81-E ships as a documentation-only update (no code change). The HEALTH_LOG 2026-05-13 entry's Issue 3 is closed as "hypothesis refuted by code inspection"; the diagnostic-line follow-up is added to NEXT_STEPS for the next recurrence.
