...
Last Updated: 2026-06-02 10:09 UTC — **RED Tue pre-market probe (5:09 AM CT).** 8/8 containers Up 40h healthy. /health 7/7 ok. 0 crash-triad. Pytest 360p/0f ✓. CI run 26748888906 on e170384 conclusion=success ✓. All APIS_* flags correct ✓. job_count=36 ✓. Mon Jun 1 ran 7 paper cycles cleanly (c1-c7); Phase 82 dedup ✓ at c7. **R1 CARRY-FORWARD unchanged**: 5 dup OPEN pairs (AMD×2/AMZN×2/INTC×2/MRVL×2/MU×2) — Phase 85 fix still pending; no new dup pairs added Mon. **NEW R2**: c7 (19:30 UTC, API process) — Alpaca paper broker returned fill_qty=0 for ALL 4 orders (INTC stop-loss close, BE trailing-stop close, GOOGL open, GOOG open). `phase80_orders_writer_qty_unresolvable` for GOOGL+GOOG. Hypothesis: API-process Alpaca adapter behaves differently from worker-process adapter. **NEW R3**: BE (May8, qty=29) DB close-loop gap — trailing_stop triggered, fill_qty=0, position still open. **NEW R4**: INTC May21 (qty=75) DB close-loop gap — stop_loss triggered (pnl=-7.70%), fill_qty=0, position still open. Both INTC rows remain open. Portfolio: cash=$52,730.07, equity=$121,506.71 (c7 Jun 1). eval_runs=113 ✓. Data fresh: bars=Jun 1, signals/rankings=Jun 1 (stale pre-market, self-heal ~10:30 UTC today). RED email draft r6247417150522250571. **Action required from Aaron**: (1) HIGH RED — Check Alpaca paper account: are BE/INTC actually closed, GOOGL/GOOG open? If yes, DB cleanup for R3/R4. Root-cause API-process fill_qty=0 issue; (2) HIGH RED — Phase 85 `_persist_positions` fix; (3) HIGH RED — DB cleanup SQL (AMD Apr29, AMZN Apr29, INTC May1, MU May1, MRVL May19); (4) MEDIUM — verify daily_opens_count SOD reset at Tue c1 13:35 UTC.
