...
Last Updated: 2026-05-21 19:10 UTC — **RED Thu 2 PM CT probe (post-c6).** **NEW (R1)**: 4 duplicate OPEN ticker rows — AMD×2, AMZN×2, INTC×2, MU×2. Root cause: c3 cycle (15:30 UTC) SOLD AMD/AMZN/INTC/MU fully closing them at the broker, but `_persist_positions` close-loop failed to update the OLD April-May DB rows to `status='closed'`. c4 cycle (16:00 UTC) re-opened all 4 (Phase 81-B guard allowed — broker showed flat). Now each ticker has 2 OPEN rows (old April-May `opened_at` + new today 16:00 `opened_at`). Cross-session close-path matching failure. Phase 85 candidate. **NEW (R2)**: `broker_health_position_drift` fired at c6 18:30 UTC with 10 tickers (AMD, BE, AMZN, INTC, MRVL, UNH, AAPL, MU, GOOGL, WDC) — direct consequence of R1 + Y2 orphan rows. **NEW (Y1)**: GOOG OPEN row created at 18:30 UTC by API-process c6 with `origin_strategy='unknown'` and no BUY order in orders table — likely broker-sync reconciliation path in `_persist_positions`; broker-sync path doesn't stamp origin_strategy correctly. **Carry-forward (Y2)**: AAPL qty=25 + MRVL qty=42 pre-Phase-82 orphan rows now Day 3; now in drift list; warrant DB cleanup. Stack runtime healthy: 8/8 containers (worker+api `Up 4h` since 15:16 UTC Phase 83/84 restart; postgres/redis/monitoring `Up 4 days`; RestartCount=0 core); /health 7/7 ok 19:08 UTC mode=paper; 0 crash-triad. Worker 0 ERR post-restart ✅ (all pre-restart stale-ticker + Phase 83 c1 tz-bug RESOLVED). API 0 ERR post-restart except broker_drift at 18:30. Prom 2/2 up; Alertmanager 0 active. Resources fine (worker 119 MiB / api 172 MiB / control-plane 1.71 GiB). DB 251 MB. Thu 6 cycles (c1-c6 all single-snapshot, Phase 82 dedup working ✅). Latest snapshot 18:30 UTC: cash=$38,098.34, equity=$119,287.72 ✅. 15/15 OPEN (4 dup tickers). NULL-qty FILLED=32 unchanged. Bars 2026-05-20 ✅; signals 2026-05-21 10:30 ✅; rankings 2026-05-21 10:45 ✅. Kill-switch=false ✅, mode=paper ✅. evaluation_runs=108 ✅. All 11 APIS_* flags correct ✅. Pytest 360p/0f ✅. Alembic q7r8s9t0u1v2 single head ✅. CI run 26235290446 on 9cb49ad conclusion=success ✅. **Action required from Aaron**: (1) HIGH RED — DB cleanup: close 4 old dup OPEN rows (AMD/AMZN/INTC/MU from April-May); SQL in HEALTH_LOG §Action Required; (2) HIGH RED — Phase 85 investigation: cross-session close-loop failure in `_persist_positions`; (3) MEDIUM — DB cleanup: close AAPL + MRVL orphan rows (Day 3); (4) LOW — stamp origin_strategy on broker-sync path. **RED Gmail draft `r-4477402235889780436` created — manual send required.**
